...
pub static TEST_ENV: Lazy<Arc<Mutex<TestEnv>>> = Lazy::new(|| {
    Arc::new(Mutex::new(TestEnv::new().expect("Failed to create test environment")))
});
...
impl TestEnv {
    ...
    fn new() -> Result<Self> {
        ...
        println!("🧪 Test environment created at: {}", temp_dir.path().display());
...
macro_rules! setup_test {
    () => {{
        ...
        println!("📍 Test using environment at: {}", env.temp_dir_path().display());
        drop(env);
        get_test_app_state!()
    }};
}