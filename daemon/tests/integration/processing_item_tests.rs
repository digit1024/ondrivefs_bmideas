...
async fn test_processing_item_modified_locally_and_remotely() -> Result<()> {
    ...
    //WHEN
    let original_item = drive_items_with_fuse_repo.get_drive_item_with_fuse_by_virtual_ino(5).await?;
    ...
    let local_processing_item = repo.get_processing_item_by_id(local_processing_item_id).await?;
    let sync_processor = onedrive_sync_daemon::sync::SyncProcessor::new(app_state);
    sync_processor.process_single_item(&remote_processing_item.unwrap()).await?;
    sync_processor.process_single_item(&local_processing_item.unwrap()).await?;
    // THEN
    // The item is NOT modified
    let item = drive_items_with_fuse_repo.get_drive_item_with_fuse_by_virtual_ino(5).await?;
    assert_eq!(item.is_some(), true);
    let item = item.unwrap();
    assert_eq!(item.drive_item().etag, original_item.unwrap().drive_item().etag); // Etag is NOT updated
    assert_eq!(item.drive_item().name, Some("Q1_Report.pdf".to_string())); // Name is not modified
    assert_eq!(item.parent_ino(), Some(4)); // Parent is not modified
    assert_eq!(item.drive_item().file.is_some(), true); // File is not modified
    // And The processing item is Conflict
    let processing_item = repo.get_processing_item_by_id(stored_id).await?;
    assert_eq!(processing_item.unwrap().status, ProcessingStatus::Conflicted);
    let processing_item = repo.get_processing_item_by_id(local_processing_item_id).await?;
    assert_eq!(processing_item.unwrap().status, ProcessingStatus::Conflicted);
