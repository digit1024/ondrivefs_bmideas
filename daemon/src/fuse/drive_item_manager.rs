...
impl DriveItemManager {
    ...
    pub async fn update_drive_item_from_file(drive_item: &mut DriveItem, file_path: &Path) -> Result<()> {
        use std::fs;
        ...
        if metadata.is_file() {
            ...
            let file_facet = crate::onedrive_service::onedrive_models::FileFacet {
                mime_type,
            };
        ...
        debug!("📂 Updated DriveItem from file metadata: {} (size: {})", 
               file_path.display(), metadata.size());
    ...
    pub fn create_temp_root_stub(drive_item_with_fuse_repo: &crate::persistency::drive_item_with_fuse_repository::DriveItemWithFuseRepository) -> DriveItemWithFuse {
