...
impl DatabaseManager {
    ...
    pub async fn get_item_by_ino(&self, ino: u64) -> Result<Option<DriveItemWithFuse>> {
        let item = crate::fuse::utils::sync_await(
        ...
        if item.is_some() {
            ...
            if ino == 1 {
                ...
                Ok(Some(
                    crate::fuse::drive_item_manager::DriveItemManager::create_temp_root_stub(
                        inner_repo,
                    ),
                ))
            } else {
    ...
    pub async fn mark_db_item_as_modified(&self, ino: u64) -> Result<()> {
        if let Ok(Some(item)) = self.get_item_by_ino(ino).await {
            ...
            debug!(
                "📝 Marked item as modified: ino={}, name={}",
                ino,
    ...
    pub async fn apply_local_change_to_db_repository(
        ...
        let inode = crate::fuse::utils::sync_await(
            self.drive_item_with_fuse_repo
                .store_drive_item_with_fuse(&item_with_fuse),
        )?;

        debug!(
            "📝 Applied local change: type={}, parent_ino={}, name={}, ino={}",
