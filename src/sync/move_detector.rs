...
mod tests {
    ...
    fn test_has_moved() {
        ...
        let move_detector = MoveDetector::new(metadata_manager);
        let cache_dir = PathBuf::from("/cache");
        
        let item = create_test_item("test.txt", "/drive/root:/Documents");
        let result = move_detector.has_moved(&item, &cache_dir);
    ...
    #[test]
    fn test_get_old_path() {
        let metadata_manager = crate::metadata_manager_for_files::get_metadata_manager_singleton();
        let move_detector = MoveDetector::new(metadata_manager);
        
        let item = create_test_item("test.txt", "/drive/root:/Documents");
        let result = move_detector.get_old_path(&item);
        
        // Should not panic and return an Option
        assert!(result.is_ok());
    }
} 